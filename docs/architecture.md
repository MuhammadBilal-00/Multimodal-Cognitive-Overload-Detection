...
| Detect | `lib/faceLandmarker.ts` | MediaPipe `FaceLandmarker`, GPU delegate with CPU fallback, self-hosted `.task` model and WASM — never hotlinked |
| Extract | `lib/features.ts` | The 13-float feature vector, ported line-for-line from CONTRACT.md §2–4. `LandmarkDebugOverlay.tsx` visualizes the exact same index constants this file exports, for the Day-3 visual verification step |
