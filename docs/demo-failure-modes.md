...
|---|---|---|---|---|
| No face in frame | Synthetic black frame (`ffmpeg color=black`) | `0.0000` | 0 | ✅ All-zero features per CONTRACT §2.1's missing-face rule. UI shows the black feed, zeroed feature panel, status stays `live` (not stuck on "filling window" or frozen) with a low-confidence-looking but well-formed prediction. **Resolved 2026-08-29** (found during the live rehearsal): the panel now suppresses model output entirely on no-face windows — engagement shows "— no face" with an explanatory notice, state bars show em-dashes, and the trend chart holds its last real points instead of spiking on the model's arbitrary output for an input it has effectively never seen (DAiSEE face-detection rate 99.96% means all-empty windows were absent from training). |
